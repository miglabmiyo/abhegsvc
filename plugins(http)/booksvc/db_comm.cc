...
bool DBComm::GetBookSearch(const std::string& key,const int64 from,const int64 count,
    ...
    //call proc_GetBooksByKey('古',0,10)
	os<<"call proc_GetBooksByKey(\'"<<key<<"\',"<<from<<","<<count<<")";
	std::string sql = os.str();
	LOG_MSG2("[%s]", sql.c_str());
	r = engine->SQLExec(sql.c_str());
